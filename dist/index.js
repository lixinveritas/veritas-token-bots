import { Client, GatewayIntentBits, Partials, ActivityType, VoiceChannel, StageChannel } from 'discord.js';
...
config.tokens.forEach((token, index) => {
    ...
    const connectToChannel = async () => {
        ...
        if (channel && (channel instanceof VoiceChannel || channel instanceof StageChannel)) {
            try {
                ...
                console.error(chalk.inverse.bgYellowBright(`Kanal katılma hatası (Kanal ID: ${channelId}): ${error.message}`));
            }
        }
        else {
    ...
    client.login(token).catch(error => {
        console.error(chalk.inverse.bgYellowBright(`Giriş hatası (Token: ${token}): ${error.message}`));
    });
});
// Komut dinleyicisi
